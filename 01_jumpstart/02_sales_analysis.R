...
bike_orderlines_wrangled_tbl <- bike_orderlines_joined_tbl %>%
    ...
    separate(location, 
        ...
        #   Mutate:  Price Extended = Qty * Price
    mutate(total.price = price * quantity) %>%
    
        #   Select: to pick what we want to keep
    select(-...1, -location) %>%
    select(-ends_with(".id")) %>%

        #   Grabbing order.id from bike_order_lines_joined_tbl to add order.id that was removed
...
sales_by_year_tbl %>%
        ...
        #Set up canvas with year (x-axis) and sales (y-axis)
